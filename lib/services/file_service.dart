...
Future<Database> get database async{
  ...
  Future<void>createTable(String tableName) async{
   ...
   final db=await database;
    List<Map<String,dynamic>> columns=await db.rawQuery("PRAGMA table_info(tailors)");
  ...
  Future<bool> checkColumnExist(String columnName)async{
    ...
    for(int i=0;i<columnsToAdd.length;i++){
      if(await checkColumnExist(columnsToAdd[i])==false){
        await db.execute('ALTER TABLE tailors ADD COLUMN ${columnsToAdd[i]} TEXT');
      }
      else
      {
...
Future<void> insertDataIntoTable(List<dynamic> keys, List<dynamic> values)async{
